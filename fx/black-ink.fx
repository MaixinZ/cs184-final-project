...
)
{
    ...
    float lumaEdge = smoothstep(BlackInk_OutlineThreshold * 0.55, BlackInk_OutlineThreshold * 1.65, length(lumaGradient));
    ...
    float lumaCue = max(lumaEdge * 0.78, broadEdge * 0.48);
...
{
    float lumaEdge = smoothstep(BlackInk_OutlineThreshold * 0.50, BlackInk_OutlineThreshold * 1.80, length(lumaGradient));
    float chromaEdge = smoothstep(0.03, 0.18, length(baseColor - smallBlur));
    return saturate((lumaEdge * 0.72 + chromaEdge * 0.28) * lerp(0.52, 1.0, detailMask));
}
...
float BlackInk_EvalContactEdge(float cavityMask, float2 lumaGradient, float shadowSignal)
{
    ...
    float compression = smoothstep(BlackInk_OutlineThreshold * 0.45, BlackInk_OutlineThreshold * 1.20, length(lumaGradient));
...
)
{
    ...
    float tone = lerp(dots * mildTone, hatch * denseTone, materialStyle);
