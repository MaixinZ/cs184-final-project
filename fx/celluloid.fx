...
float Celluloid_SampleBlurredLuma(float2 uv, float2 texel)
{
    return Celluloid_Luminance(Celluloid_SampleRegionAverage(uv, texel));
}

float2 Celluloid_EvalCentralGradient(float2 uv, float2 texel)
{
    float lL = Celluloid_SampleBlurredLuma(uv - float2(texel.x, 0.0), texel);
    float lR = Celluloid_SampleBlurredLuma(uv + float2(texel.x, 0.0), texel);
    float lU = Celluloid_SampleBlurredLuma(uv + float2(0.0, texel.y), texel);
    float lD = Celluloid_SampleBlurredLuma(uv - float2(0.0, texel.y), texel);
    return float2(lR - lL, lU - lD);
}

float2 Celluloid_EvalSobelGradient(float2 uv, float2 texel)
{
    float tl = Celluloid_SampleBlurredLuma(uv + float2(-texel.x, texel.y), texel);
    float tc = Celluloid_SampleBlurredLuma(uv + float2(0.0, texel.y), texel);
    float tr = Celluloid_SampleBlurredLuma(uv + texel, texel);
    float ml = Celluloid_SampleBlurredLuma(uv - float2(texel.x, 0.0), texel);
    float mr = Celluloid_SampleBlurredLuma(uv + float2(texel.x, 0.0), texel);
    float bl = Celluloid_SampleBlurredLuma(uv - texel, texel);
    float bc = Celluloid_SampleBlurredLuma(uv - float2(0.0, texel.y), texel);
    float br = Celluloid_SampleBlurredLuma(uv + float2(texel.x, -texel.y), texel);
...
float2 Celluloid_EvalScharrGradient(float2 uv, float2 texel)
{
    float tl = Celluloid_SampleBlurredLuma(uv + float2(-texel.x, texel.y), texel);
    float tc = Celluloid_SampleBlurredLuma(uv + float2(0.0, texel.y), texel);
    float tr = Celluloid_SampleBlurredLuma(uv + texel, texel);
...
float2 Celluloid_EvalLumaGradient(float2 uv, float2 texel)
{
    if (Celluloid_GradientMode == 1)
        return Celluloid_EvalSobelGradient(uv, texel);
    if (Celluloid_GradientMode == 2)
        return Celluloid_EvalScharrGradient(uv, texel);
    return Celluloid_EvalCentralGradient(uv, texel);
}
